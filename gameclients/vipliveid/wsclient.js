...
var appKey = 'X2L2tEA5Qfvd6xb3NtqzN9jbjNAswpKF6h5L';
var appToken;
...
function onConnectClick() {
	...
	var ws_protocol = 'wss';
	var ws_hostname = 'api-active.viplive.co';
	var ws_port     = 8443;
	var ws_endpoint = '/socket.io/?EIO=3&transport=websocket&application-key=' + appKey + '&session-token=' + appToken;
...
function openWSConnection(protocol, hostname, port, endpoint) {
	...
	try {
		...
		webSocket.onopen = function(openEvent) {
			...
			sendMessage('40/iq-trivia?application-key=' + appKey + '&session-token=' + appToken);
		};
...
function search(question, options) {
	...
	keyword = question + ' ("' + options.join('" OR "') + '")';
